import { HSurvey } from "@/constants-temp/data";
...
const HomeSurvey = () => {
  return (
    <section className="relateive h-survey mt-[-4rem] relative z-10">
      <div className="container mx-auto flex flex-col">
        <div className="flex bg-white pt-[2.3rem] pb-14 flex-col items-center justify-center relative h-survey__wrap">
          <div className="h-survey__wrap-decor absolute top-0 left-[50%] transform -translate-1/2 w-[63px] h-[63px] rounded-[50%] flex flex-col items-center justify-center bg-tosca">
            <svg
              width="26"
              height="33"
              viewBox="0 0 26 33"
          ...
          <div className="flex flex-col w-full border-b-1 border-[#00529C33] pb-6">
          ...
          <h2 className="text-navyblue text-6xl text-center w-[70%] mt-10 mb-6">
            {HSurvey.title}
          </h2>
          <div className="flex flex-row h-survey__wrap-btn gap-4">
            {HSurvey.btnSurvey.map((item, index) => (
              <ButtonPrimary
                key={index}
                href={item.href}
                target={item.target}
