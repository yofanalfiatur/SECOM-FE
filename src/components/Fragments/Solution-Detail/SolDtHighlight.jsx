...
const SolDtHighlight = ({ translationKey, SolDetailDocument }) => {
  const t = useTranslations();
  const SolDtHighlight = t.raw(translationKey);
  const SolDtDocument = t.raw(SolDetailDocument);
  const locale = useLocale();
  const isDesktop = useIsDesktop();

  return (
    <section className="flex flex-col relative pt-8 lg:pt-36 pb-10 lg:pb-26 overflow-hidden sol-dt-highlight">
      <BackgroundDots
      ...
      <div className="container mx-auto relative z-[1]">
        <h2 className="text-darkblue font-raleway font-normal text-[30px] lg:text-[40px] text-center leading-[1.3] lg:leading-[1.2]mb-8 lg:mb-11">
          {locale === "en" ? "Product Highlight" : "Sorotan Produk"}
        </h2>
        <div className="flex flex-col gap-y-6">
          {SolDtHighlight.map((item, index) => (
            <div
              className="flex flex-row [&:nth-child(even)]:flex-row-reverse group"
              key={index}
            >
              ...
              <div className="w-full lg:w-5/12 flex flex-col justify-center group-[&:nth-child(odd)]:lg:pl-15 group-[&:nth-child(even)]:lg:pr-15">
                <p className="text-darkblue font-raleway font-normal text-[20px] lg:text-[35px]">
                  {item.title}
                </p>
                <p className="text-darkblue text-sm font-normal leading-[1.7] lg:leading-[1.5] mt-2 lg:mt-3 lg:text-[18px]">
                  {item.desc}
                </p>
              </div>
            </div>
      ...
      <div className="container mx-auto relative z-[1] mt-10 lg:mt-28 bg-navyblue flex flex-col lg:flex-row">
        <div className="w-full lg:w-4/12 flex flex-col items-center relative">
          <Image
            src={SolDtDocument.image}
            width={171}
            height={171}
            quality={100}
            alt={SolDtDocument.title}
            className=" object-contain relative z-[1] mt-[-10%]"
          />
          <div
            className="bg-tosca w-[130%] h-full absolute bottom-0 left-1/2 -translate-x-1/2"
            style={{
              clipPath: "circle(50% at 50% 200%)",
            }}
          ></div>
        </div>
        <div className="w-full lg:w-5/12 flex flex-col justify-center">
          <p className="text-white font-raleway font-normal text-[20px] lg:text-[30px]">
            {SolDtDocument.title}
          </p>
        </div>
        <div className="w-full lg:w-3/12 flex flex-col justify-center items-end lg:pr-6">
          <Link
            href="#document"
            target="_blank"
            className="flex flex-row bg-tosca rounded-[5px] max-w-max px-5 py-4 gap-3 transition-all ease duration-200 hover:bg-white group"
          >
            <svg
              width="27"
              height="28"
