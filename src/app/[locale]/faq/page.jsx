...
const FAQPage = () => {
  ...
  const FAQPageData = t.raw("FAQPage");
  const isDesktop = useIsDesktop();
  const locale = useLocale();

  const uniqueCategories = [...new Set(FAQPageData.map((faq) => faq.category))];
  const [activeTab, setActiveTab] = useState(uniqueCategories[0]);

  const [activeIndex, setActiveIndex] = useState(null);

  const filteredFaqs = FAQPageData.filter((faq) => faq.category === activeTab);
              ...
              {uniqueCategories.map((category, index) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    setActiveIndex(null); // reset accordion saat pindah kategori
                  }}
                >
                  {category}
                </button>
